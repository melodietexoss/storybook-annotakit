...
NATIVE toolbar buttons (pin · region · threads+count · show/hide) + ⌥-hotkeys (`⌥C` pin · `⌥R` region · `⌥L` show/hide · `⌥D` drawer · `?` help card). Canvas must be focused; keys match by PHYSICAL `e.code` (Option-compose can't break them); legacy plain-key configs work with ⌥ held. Esc cancels; ⌘/Ctrl+Enter submits. Composer shows the EXACT element identity live — the same string the digest renders. Bottom dock → "Annotakit" panel: threads, reply, resolve/reopen, exports, sync status, issue links, 📷 dom chip → snapshot html (§3). Nothing alters the canvas DOM (passive badge only if the API is down).
...
curl -X PATCH $API/threads/<id> -H 'content-type: application/json' -d '{"status":"resolved"}'
...
The loop: GET threads → `component.source.file:line` + `story.importPath` → fix the code → reply with evidence → PATCH `{"status":"resolved"}` (partial OK — JSON-merge server-side) → reviewer re-checks visually (pins re-anchor; no headless check). `resolvedAt` is stamped server-side on open→resolved even if omitted; a PATCH omitting server-held comments (imported replies) is MERGED — stale snapshots never drop data.
...
| PATCH status | validated enum (`open`/`resolved`, case-insensitive; else 400 — `resolvedAt` only on real open→resolved) |
...
**Path A — GitHub-only loop** (no localhost access): list issues labeled `annotakit` → body (component, repo-root-relative `jsx:` path, element text, selector, thread id) → fix code → comment with evidence (diff + SHA) → close. The reviewer's thread resolves within one poll (60s), comments imported as replies (`source: "github"`); issue-reopen re-opens the thread; post-close comments still import.
...
- **ALWAYS-MERGE**: pushed tree = local ∪ remote — a fast-forward push can never clobber another machine's threads (union by thread id; delete-wins tombstones; resolved-wins; comment union; gh-mapping either-side; counters max+1; an all-deleted store still publishes). NEVER force-push; CAS update-ref only after a successful push.
