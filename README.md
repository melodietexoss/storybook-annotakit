...
- **Resolve programmatically (Path B)**: `GET /api/threads` → take the thread OBJECT → set `"status":"resolved"` → `PATCH` the FULL object to `/annotakit/api/threads/<id>` (the server stamps `resolvedAt` on transitions; stale snapshots are merged server-side — omitted comments are never dropped). The lighter partial form works too: `PATCH` with just `{"status":"resolved"}` (JSON-merge semantics, same field guarantees) — GET /schema documents both.
...
node scripts/api-test.mjs          # 67/67 contract tests (run from repo root, needs the dev server up)
node scripts/ghsync-fake.mjs       # 61/61 lifecycle + stress engine tests (no server needed)
node scripts/ghclient-test.mjs     # 55/55 client-side publisher tests (no server needed)
```
